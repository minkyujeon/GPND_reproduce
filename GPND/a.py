...
def main(test_fold_id, valid_fold_id, subject_folder, gpu_num, inliner_classes, total_classes, batch_size, zsize, percentages, threshold_percent_list, folds=5):
    global use_cuda
    ...
    def compute_threshold(mnist_valid, percentage, threshold_percent_list):
        #############################################################################################
        ...
        return result, [(np.quantile(result, thres_p/100), thres_p) for thres_p in threshold_percent_list]

    def test(mnist_test, percentage, e, threshold_percent, valid_result):
        # true_positive = 0
        # true_negative = 0
        # false_positive = 0
        # false_negative = 0
        ...
        test_result_dict = {
            "inlier": {"result":[], "true_positive": 0, "true_negative": 0, "false_positive": 0, "false_negative": 0},
            "outlier": {"result":[], "true_positive": 0, "true_negative": 0, "false_positive": 0, "false_negative": 0}
        }
            ...
            for i in range(batch_size):
                ...
                if (label[i].item() in inliner_classes) == (P > e):
                    if label[i].item() in inliner_classes:
                        test_result_dict['inlier']['true_positive'] += 1
                    else:
                        test_result_dict['inlier']['true_negative'] += 1
                else:
                    if label[i].item() in inliner_classes:
                        test_result_dict['inlier']['false_negative'] += 1
                    else:
                        test_result_dict['inlier']['false_positive'] += 1
                test_result_dict['inlier']['result'].append(
                    ((label[i].item() in inliner_classes), P)
                )

                #
                # outlier가 target인 경우
                #
                if (label[i].item() in outlier_classes) == (P < e):
                    if label[i].item() in outlier_classes:
                        test_result_dict['outlier']['true_positive'] += 1
                    else:
                        test_result_dict['outlier']['true_negative'] += 1
                else:
                    if label[i].item() in outlier_classes:
                        test_result_dict['outlier']['false_negative'] += 1
                    else:
                        test_result_dict['outlier']['false_positive'] += 1
                test_result_dict['outlier']['result'].append(
                    ((label[i].item() in outlier_classes), P)
                )

        # with open(os.path.join(subject_folder, 'result_d%d_p%d.pkl' % (outlier_classes[0], percentage)), 'wb') as output:
        #     pickle.dump(result, output)

        for standard, values_dict in test_result_dict.items():
            test_result = values_dict['result']
            true_positive = values_dict['true_positive']
            true_negative = values_dict['true_negative']
            false_positive = values_dict['false_positive']
            false_negative = values_dict['false_negative']

            error = 100 * (true_positive + true_negative) / count

            y_true = [x[0] for x in test_result]
            y_scores = [x[1] for x in test_result]
            ...
            # valid set threshold 기준으로 True라고 간주한 test set들
            X1 = [x[1] for x in test_result if x[0]]

            # valid set threshold 기준으로 False라고 간주한 test set들
            Y1 = [x[1] for x in test_result if not x[0]]

            minP = min([x[1] for x in test_result]) - 1
            maxP = max([x[1] for x in test_result]) + 1
            ...
            with open(os.path.join(subject_folder, "results_{}_{}.txt".format(standard, str(threshold_percent).replace(".", "_"))), "a") as file:
                file.write(
                    "In_or_Out class: %d\n Threshold percentage: %f\n Standard: %d\n Outlier Percentage: %d\n"
                    "Error: %f\n F1: %f\n AUC: %f\nfpr95: %f"
                    "\nDetection: %f\nauprin: %f\nauprout: %f\n\n" %
                    (
                        outlier_classes[0], threshold_percent, standard, percentage,
                        error, f1, auc, fpr95, error, auprin, auprout
                    )
                )
    ...
    results = {}
    e = compute_threshold(mnist_valid)
    for percent in percentages:

        results[percent] = test(mnist_test, percent, e)

        valid_result, threshold_and_percent_list = compute_threshold(mnist_valid, percent, threshold_percent_list)
        for threshold, threshold_percent in threshold_and_percent_list:
            results[percent] = test(mnist_test, percent, threshold, threshold_percent, valid_result)
...
if __name__ == '__main__':
    ...
    main(
        test_fold_id=0, valid_fold_id=1, subject_folder="test_fold_0", gpu_num=1,
        inliner_classes=inliner_classes, total_classes=10, batch_size=1024, zsize=32,
        percentages=[5, 10, 15, 20, 25, 30, 35], threshold_percent_list=[95, 96.5, 98, 99.5]
    )
