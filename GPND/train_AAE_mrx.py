...
def validate(
    model_and_opt_dict, data_valid_x, batch_size, BCE_loss,
    ...
    with torch.no_grad():
        ...
        for batch_it in range(len(data_valid_x) // batch_size):
            
            if data_valid_x.shape[1] == 3: #to gray scale
                x = extract_batch(data_valid_x, batch_it, batch_size)#.view(-1, 1, 32, 32)
                x = x.mean(axis=1)
...
def train_an_epoch(
    model_and_opt_dict, data_train_x, batch_size, BCE_loss,
    y_real_, y_fake_, y_real_z, y_fake_z, zsize,
    ...
    for batch_it in range(len(data_train_x) // batch_size):
        if data_train_x.shape[1] == 3: #to gray scale
            x = extract_batch(data_train_x, batch_it, batch_size)#.view(-1, 1, 32, 32)
            x = x.mean(axis=1)
            x = x.unsqueeze(axis=1)
        else:
            x = extract_batch(data_train_x, batch_it, batch_size).view(-1, 1, 32, 32)
...
def main(test_fold_id, valid_fold_id, subject_folder, gpu_num, modal, num_epoch, dataset, inliner_classes, total_classes, batch_size, zsize, folds=5):
    global use_cuda
    ...
    print('inliner_classes:',inliner_classes)
    ...
    data_train_x, data_train_y = list_of_pairs_to_numpy(data_train) #중간에 image data가 아닌 함수(PIL)가 껴있음
    data_valid_x, data_valid_y = list_of_pairs_to_numpy(data_valid)
...
if __name__ == '__main__':
    ...
    main(
        test_fold_id=0, valid_fold_id=1, subject_folder="newly_test", gpu_num=0, modal=modal, num_epoch=10,
        dataset='mnist', inliner_classes=lst, total_classes=10, batch_size=128, zsize=32,
    )
