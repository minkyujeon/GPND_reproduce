import train_AAE_mrx
import novelty_detector_mrx
import csv
import random
import torch
...
if __name__ == '__main__':
    ...
    try:
        dataset = arg_list[3].lower() #dataset : mnist / fmnist / cifar
    except:
        raise Exception("You missed dataset!")
    ...
    try: 
       modal = arg_list[5].lower() #unimodal / multimodal 
    except:
        raise Exception("You must choice unimodal or multimodal")
    ...
    for i in range(10):
        if modal == 'multimodal':
            ...
            lst = [i]
        train_AAE_mrx.main(
            test_fold_id, valid_fold_id, subject_folder, gpu_num, modal=modal,
            dataset=dataset, num_epoch=num_epoch, inliner_classes=lst, total_classes=10, batch_size=256, zsize=32
        )
        res = novelty_detector_mrx.main(
            test_fold_id, valid_fold_id, subject_folder, gpu_num, modal=modal,
            dataset=dataset, inliner_classes=lst, total_classes=10, batch_size=256, zsize=32, percentages=percentages
        )
