...
def compute_jacobian(inputs, output):
    ...
    for i in range(num_classes):
        ...
        # print('inputs.grad.data:',inputs.grad.data)
...
def main(test_fold_id, valid_fold_id, subject_folder, gpu_num, is_fashion, inliner_classes, total_classes, batch_size, zsize, percentages, folds=5):
    global use_cuda
    ...
    #print('mnist_train:',mnist_train[0][1].shape) #[32,32]
    ...
    plt.savefig(os.path.join(subject_folder, 'mnist_d%d_randomsearch.pdf' % inliner_classes[0]))
    plt.savefig(os.path.join(subject_folder, 'mnist_d%d_randomsearch.eps' % inliner_classes[0]))
    plt.clf()
    plt.cla()
    plt.close()
    ...
    plt.yticks(fontsize=ticks_size)
    plt.tight_layout(rect=(0.0, 0.0, 1, 0.95))
    plt.savefig(os.path.join(subject_folder, 'mnist_d%d_embeding.pdf' % inliner_classes[0]))
    plt.savefig(os.path.join(subject_folder, 'mnist_d%d_embeding.eps' % inliner_classes[0]))
    plt.clf()
    plt.cla()
    plt.close()
    ...
    def compute_threshold(mnist_valid, percentage):
        ...
        for it in range(len(mnist_valid_x) // batch_size):
            print('mnist_valid:',mnist_valid_x.shape)
...
if __name__ == '__main__':
    ...
    main(
        ...
        is_fashion = False, inliner_classes=[0], total_classes=10, batch_size=64, zsize=32, percentages=[35]
    )
